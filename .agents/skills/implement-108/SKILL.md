...
Run the assigned next unchecked task only. Order: T01 welcome fact, T02 overview `jobId`, T03 reload reconnect, T04 prototype gate, T05a keep `lastSyncedAt` as the last completion (sync-engine, merges before T05), T05 state and wizard, T05b Coinbase connect step (held behind #139; skip to T06 while it is held), T06 billing return, T07 welcome step, T08 browser verification, T09 Harvest with the `harvest` skill. T01–T04 are merged or approved; the maintainer approved variant D (full-screen letter wizard) on 2026-09-08 and amendment 1 added the `needs_source` state and T05b; read the live body. Tasks are sequential from merged `main`; refuse a task whose predecessor is unmerged and say why.
...
Refresh open PRs at pickup. Active streams: #133 (verified email sign-up, armed at `9dc2df29e0`) shares the auth surface with this spec: `packages/rest-api/src/definitions/AuthApi.ts`, `packages/rest-api/src/layers/AuthApiLive.ts`, `packages/rest-api/tests/AuthApiLive.integration.test.ts`, `packages/persistence/src/schema/UsersTable.ts`, `packages/sdk/src/auth/index.ts`, and `apps/www/src/routes/app.tsx` for its `/app` redirect; its T01 and T04 carry migrations, so check `gh pr list` for an open migration PR before any schema work here. #127 T04/T05 (transaction inspector) may open PRs touching `apps/www/src/components/transactions-table.tsx`, `apps/www/src/components/dashboard.tsx`, `apps/www/src/integrations/taxmaxi/queries.ts`, and `apps/www/src/routes/app.tsx`. #269 is harvested. Preserve others' edits in shared files and report out-of-surface conflicts. PR #210 (Postgres worker queue) is parked reference material with its own migration; it is not an active stream. PRs #179 and #176 are docs drafts. T01 carries this spec's only migration.
