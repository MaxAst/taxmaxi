        ...
        Effect.gen(function* () {
          ...
          if (active !== undefined) {
            const [reused] = yield* tx
              ...
              .set({ lastSentAt: start.sentAt.toDate(), updatedAt: now })
            ...
            // `consume` does not take the user lock, so a verify can delete the
            // row between the select and this update; then start fresh below.
