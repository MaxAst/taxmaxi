...
The limits stay tight when the email provider is flaky: after a provider error the person waits until the cooldown ends or the request expires, whichever comes first, because a reused request keeps its original `expiresAt` and a login after expiry starts a fresh request at once. The failed delivery uses one of the five sends only when it was a first send or an explicit resend, because a login-triggered reuse of an active request does not advance `send_count`. That is the accepted cost; the limits guard against spam and brute force, and a provider error is rare.
