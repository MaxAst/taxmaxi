...
export const FIRST_SYNC_PROTOTYPE_VARIANTS = ["a", "b", "c"] as const
...
export const firstSyncPrototypeSearchSchema = z.object({
  ...
  state: z.enum(FIRST_SYNC_PROTOTYPE_STATES).optional().catch(undefined),
})
...
const STATE_LABELS: Record<FirstSyncPrototypeState, string> = {
...
/* ─────────────────────────────────────────────────────────
 * Entry point rendered by the dashboard inside the content sheet.
 * ───────────────────────────────────────────────────────── */
...
export function FirstSyncBodyPrototype({
  ...
  const mock = getFirstSyncPrototypeMock(state, targetSource)
  const model = { billing: mock.billing, sourceName: targetSource.name, state }
  ...
  return (
    <>
      {variant === "a" ? <VariantGuidedPanel {...variantProps} /> : null}
      {variant === "b" ? <VariantSteppedEmptyState {...variantProps} /> : null}
      {variant === "c" ? <VariantGuideRailPreview {...variantProps} /> : null}
      <PrototypeSwitcher
        mock={mock}
        onExit={() => setSearch({ state: undefined, variant: undefined })}
        onSelectState={(next) => setSearch({ state: next })}
        onSelectVariant={(next) => setSearch({ variant: next })}
        state={state}
        variant={variant}
      />
    </>
...
function PrototypeSwitcher({
  ...
  onExit: () => void
  onSelectState: (state: FirstSyncPrototypeState) => void
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-50 flex -translate-x-1/2 flex-col items-center gap-1.5 rounded-2xl bg-zinc-950 px-3 py-2 font-interface text-white shadow-xl ring-1 ring-white/10">
      ...
      <div className="flex flex-wrap items-center justify-center gap-1">
        {FIRST_SYNC_PROTOTYPE_STATES.map((candidate) => (
          <button
            aria-pressed={candidate === state}
            className={cn(
              "rounded-full px-2 py-0.5 text-[0.625rem] font-medium",
              candidate === state ? "bg-white text-zinc-950" : "text-white/60 hover:bg-white/10"
            )}
            key={candidate}
